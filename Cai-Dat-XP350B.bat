@echo off
chcp 65001 >nul
title Cai Dat Phan Mem In Tem San Pham XP-350B
...
echo ========================================================
echo   CAI DAT PHAN MEM IN TEM SAN PHAM 2 HANG XP-350B
echo ========================================================
echo.
...
echo del /F /Q "%PUBLIC%\Desktop\In Tem San Pham XP-350B.lnk" ^>nul 2^>^&1
echo del /F /Q "%USERPROFILE%\Desktop\In Tem San Pham XP-350B.lnk" ^>nul 2^>^&1
echo del /F /Q "%PUBLIC%\Desktop\In Tem Tien Uyen (XP-350B).lnk" ^>nul 2^>^&1
echo del /F /Q "%USERPROFILE%\Desktop\In Tem Tien Uyen (XP-350B).lnk" ^>nul 2^>^&1
...
reg add "%REG_HKLM%" /v "DisplayName" /t REG_SZ /d "In Tem Sản Phẩm 2 Hàng (Xprinter XP-350B)" /f >nul 2>&1
...
reg add "%REG_HKLM%" /v "Publisher" /t REG_SZ /d "Open Source Community" /f >nul 2>&1
...
powershell -Command ^
  ...
  "$sc = $ws.CreateShortcut([System.Environment]::GetFolderPath('Desktop') + '\In Tem San Pham XP-350B.lnk'); " ^
  ...
  "  $scPub = $ws.CreateShortcut($pubDesktop + '\In Tem San Pham XP-350B.lnk'); " ^
